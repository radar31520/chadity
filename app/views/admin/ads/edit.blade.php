        ...
        <div class="tab-content">
            ...
            <div class="tab-pane active" id="tab-general">
                ...
                <div class="control-group {{{ $errors->has('name') ? 'error' : '' }}}">
                    ...
                    <div class="controls">
                        <input type="text" name="name" id="name" value="{{{ Input::old('name', $ad->name) }}}" />
                    ...
                    <div class="controls">
                        <select name="advertiser" id="advertiser">
                                @foreach ($advertisers as $advertiser)
                                        <option value="{{{ $advertiser->id }}}"{{{ ( $advertiser->id == $ad->advertiser_id ? ' selected="selected"' : '') }}}>{{{ $advertiser->name }}}</option>
                                @endforeach
                        </select>
                    ...
                    <div class="controls">
                        <select name="type" id="type">
                                @foreach ($types as $type)
                                        <option value="{{{ $type->id }}}"{{{ ( $type->id == $ad->type_id ? ' selected="selected"' : '') }}}>{{{ $type->name }}}</option>
                                @endforeach
                        </select>
                        <span class="help-block">
                ...
                <div class="control-group {{{ $errors->has('description') ? 'error' : '' }}}">
                    ...
                    <div class="controls">
                        <textarea class="full-width span10 wysihtml5" name="description" value="description" rows="10">{{{ Input::old('description', $ad->description) }}}</textarea>
